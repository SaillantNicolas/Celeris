...
const CreateInterventionScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    if (!form.client || !form.address || !form.scheduled_date) {
      Alert.alert('Champs obligatoires', 'Veuillez remplir les champs client, adresse et date');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...form,
        scheduled_date: form.scheduled_date.toISOString().slice(0, 19).replace('T', ' ')
      };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <TextInput style={styles.input} value={form.client} onChangeText={(val) => handleChange('client', val)} />
        ...
        <TextInput style={styles.input} value={form.issue} onChangeText={(val) => handleChange('issue', val)} />
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: '#1F2631', paddingTop: 50, paddingBottom: 15, paddingHorizontal: 20
  },
  headerTitle: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  content: { padding: 16 },
  label: { marginTop: 15, marginBottom: 5, color: '#333', fontWeight: '600' },
  input: {
    backgroundColor: '#fff', borderWidth: 1, borderColor: '#ccc', padding: 10,
    borderRadius: 5, marginBottom: 10
  },
  pickerWrapper: {
    backgroundColor: '#fff', borderWidth: 1, borderColor: '#ccc', borderRadius: 5,
    marginBottom: 20
  }
});
