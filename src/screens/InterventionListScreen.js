...
const InterventionListScreen = ({ navigation }) => {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        navigation.navigate("EditIntervention", { intervention: item })
      }
    >
      <Text style={styles.client}>{item.client}</Text>
      <Text style={styles.details}>
        {new Date(item.scheduled_date).toLocaleString()} • {item.status}
      </Text>
    ...
    <View style={styles.container}>
      <View style={styles.headerRow}>
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Ionicons name="arrow-back" size={28} color="#1F2631" />
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 20, fontWeight: "bold", color: "#1F2631" },
  input: {
    borderWidth: 1,
    borderColor: "#ccc",
    borderRadius: 6,
