...
export const getBase64FromImageUri = async (imageUri) => {
  try {
    ...
    // Pour les URIs qui commencent déjà par 'data:', on a déjà du base64
    if (imageUri.startsWith('data:')) {
      // Extraire la partie base64 de l'URI data
    ...
    if (Platform.OS !== 'web') {
      ...
      console.log('Image convertie en base64 (mobile), longueur:', base64.length);
      return base64;
    } 
    // Sur web, cette fonction ne sera pas utilisée car les URIs sont déjà en data:image format
    else {
      throw new Error('Sur le web, l\'URI devrait déjà être au format data:image');
    }
  } catch (error) {
    ...
    const response = await axios.post(
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [
          {
            role: "system",
            content: "Vous êtes un plombier expert qui analyse des problèmes de plomberie à partir d'images."
          },
          {
            role: "user",
            content: content
    ...
    const response = await axios.post(
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [
          {
            role: "system",
            content: "Vous êtes un plombier expert qui rédige des rapports sur les interventions réalisées. Soyez professionnel et technique."
          },
          {
            role: "user",
            content: content
...
// Reformuler un texte avec l'API OpenAI
    ...
    const response = await axios.post(
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [
          {
            role: "system",
            content: "Vous êtes un expert en plomberie. Votre tâche est de reformuler le texte fourni dans un langage professionnel et technique approprié pour un rapport d'intervention. Ne fournis pas de conseils"
          },
          {
            role: "user",
            content: text
