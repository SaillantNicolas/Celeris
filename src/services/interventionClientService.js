...
export const getUpcomingInterventions = async () => {
  try {
    ...
    console.error(
      "Get upcoming interventions error:",
      error.response?.data || error.message
    );
    throw (
      error.response?.data?.error || "Error fetching upcoming interventions"
    );
  }
};
...
export const getOngoingInterventions = async () => {
  try {
    ...
    console.error(
      "Get ongoing interventions error:",
      error.response?.data || error.message
    );
...
export const getInterventionHistory = async (limit = 5) => {
  try {
    const response = await apiClient.get(
      `/interventions/history?limit=${limit}`
    );
    return response.data;
  } catch (error) {
    console.error(
      "Get intervention history error:",
      error.response?.data || error.message
    );
...
export const updateInterventionStatus = async (id, status) => {
  try {
    const response = await apiClient.patch(`/interventions/${id}/status`, {
      status,
    });
    return response.data;
  } catch (error) {
    console.error(
      "Update intervention status error:",
      error.response?.data || error.message
    );
